...
async def test_run_macro_reviewer_iteration_1_rewrite(mock_llm: MagicMock):
    ...
    mock_ainvoke.return_value = fake_response
    ...
    result = await reviewer.run_macro_reviewer_subgraph_node(state, {})
    ...
    sent_messages = mock_structured_llm.ainvoke.call_args[0][0]
...
@pytest.mark.asyncio
@patch('app.agents.reviewer.llm')
async def test_run_macro_reviewer_needs_facts(mock_llm: MagicMock):
    mock_structured_llm = MagicMock()
    fake_response = FirstReviewerResponse(
...
@pytest.mark.asyncio
@patch('app.agents.reviewer.llm')
async def test_run_macro_reviewer_intermediate_iteration(mock_llm: MagicMock):
    mock_structured_llm = MagicMock()
    ...
    await reviewer.run_macro_reviewer_subgraph_node(state, {})
...
@pytest.mark.asyncio
@patch('app.agents.reviewer.llm')
async def test_run_macro_reviewer_final_iteration(mock_llm: MagicMock):
    mock_structured_llm = MagicMock()
    fake_response = NoFactsFirstReviewerResponse(
        feedback="Accepted.",
